...
def create_pretraining_dataset(input_file, max_pred_length, shared_list, args, worker_init_fn, synthetic_input=False):
    if synthetic_input: # synthetic, in-memory dataset only for performance testing
        train_data = synthetic_dataset(input_file=input_file, max_pred_length=max_pred_length, max_seq_length=args.max_seq_length, packed_samples=args.packed_samples)
    else:
        train_data = pretraining_dataset(input_file=input_file, max_pred_length=max_pred_length, max_seq_length=args.max_seq_length, packed_samples=args.packed_samples, order_samples=args.order_samples)
...
class synthetic_dataset(Dataset):
    def __init__(self, input_file, max_pred_length, max_seq_length, number_of_samples=100, packed_samples=False):
        self.max_pred_length = max_pred_length
        self.max_seq_length = max_seq_length
        self.samples = []
        ...
        for _ in range(number_of_samples):
            ...
            input_mask= np.zeros((self.max_seq_length)).astype(np.int64)
            segment_ids=np.zeros((self.max_seq_length)).astype(np.int64)
            packed_input_len = np.array([self.max_seq_length, 0, 0]).astype(np.int64)
            ...
            if not self.packed_samples:
                ...
                next_sentence_labels=np.array([1,1,1]).astype(np.int64)
                self.samples.append([torch.from_numpy(input_ids), torch.from_numpy(segment_ids),
                    torch.from_numpy(input_mask), torch.from_numpy(masked_lm_labels), torch.from_numpy(next_sentence_labels), torch.from_numpy(packed_input_len)])
...
def parse_arguments():
    ...
    parser.add_argument("--eval_cuda_graph",
                        default=False,
                        action='store_true',
                        help="Enable CUDA graph for validation.")
    ...
    parser.add_argument('--use_transformer_engine',
                        default=False,
                        action='store_true',
                        help='Enable FP8 layers using TE TransformerLayer')
    ...
    assert args.init_checkpoint is not None or args.init_tf_checkpoint is not None or found_resume_checkpoint(args) or args.synthetic_input, \
    ...
    if not args.packed_samples:
        ...
        args.order_samples = False
    if args.packed_samples and args.order_samples:
        assert args.max_pack_factor == 3, \
        "Currently ordering assumes using default packed dataset with max_pack_factor=3"
...
def remap_te_transformer_layer_parameters(model_dict, config):
    res_dict = OrderedDict()
    for k in model_dict:
        if 'attention.output.dense.weight' in k:
            new_k = k.replace('attention.output.dense.weight', 'self_attention.proj.weight')
        elif 'attention.output.dense.bias' in k:
            new_k = k.replace('attention.output.dense.bias', 'self_attention.proj.bias')
        elif 'attention.self.query.weight' in k:
            new_k = k.replace('attention.self.query.weight', 'self_attention.query.weight')
        elif 'attention.self.query.bias' in k:
            new_k = k.replace('attention.self.query.bias', 'self_attention.query.bias')
        elif 'attention.self.key.weight' in k:
            new_k = k.replace('attention.self.key.weight', 'self_attention.key.weight')
        elif 'attention.self.key.bias' in k:
            new_k = k.replace('attention.self.key.bias', 'self_attention.key.bias')
        elif 'attention.self.value.weight' in k:
            new_k = k.replace('attention.self.value.weight', 'self_attention.value.weight')
        elif 'attention.self.value.bias' in k:
            new_k = k.replace('attention.self.value.bias', 'self_attention.value.bias')
        elif 'attention.output.LayerNorm.weight' in k:
            new_k = k.replace('attention.output.LayerNorm.weight', 'layernorm_mlp.layer_norm_weight')
        elif 'attention.output.LayerNorm.bias' in k:
            new_k = k.replace('attention.output.LayerNorm.bias', 'layernorm_mlp.layer_norm_bias')
        elif 'intermediate.dense.weight1' in k:
            new_k = k.replace('intermediate.dense.weight1', 'layernorm_mlp.fc1_weight')
        elif 'intermediate.dense.bias1' in k:
            new_k = k.replace('intermediate.dense.bias1', 'layernorm_mlp.fc1_bias')
        elif 'intermediate.dense.weight2' in k:
             new_k = k.replace('intermediate.dense.weight2', 'layernorm_mlp.fc2_weight')
        elif 'intermediate.dense.bias2' in k:
            new_k = k.replace('intermediate.dense.bias2', 'layernorm_mlp.fc2_bias')
        elif 'output.LayerNorm.weight' in k:
            new_k = k.replace('output.LayerNorm.weight', 'layernorm.weight')
        elif 'output.LayerNorm.bias' in k:
            new_k = k.replace('output.LayerNorm.bias', 'layernorm.bias')
        else:
            new_k = k
        res_dict[new_k] = model_dict[k]
    model_dict.clear()
...
def remap_transformer_layer_parameters(model_dict, config):
    res_dict = OrderedDict()
    for k in model_dict:
        if 'attention.output.dense.weight' in k:
            new_k = k.replace('attention.output.dense.weight', 'attention.proj_weight')
        elif 'attention.output.dense.bias' in k:
            new_k = k.replace('attention.output.dense.bias', 'attention.proj_bias')
        elif 'attention.output.LayerNorm.weight' in k:
            new_k = k.replace('attention.output.LayerNorm.weight', 'layernorm_mlp.layer_norm_weight')
        elif 'attention.output.LayerNorm.bias' in k:
            new_k = k.replace('attention.output.LayerNorm.bias', 'layernorm_mlp.layer_norm_bias')
            ...
            new_k = k.replace('output.LayerNorm.weight', 'output_LayerNorm.weight')
        elif 'output.LayerNorm.bias' in k:
            new_k = k.replace('output.LayerNorm.bias', 'output_LayerNorm.bias')
        else:
            new_k = k
        res_dict[new_k] = model_dict[k]
    model_dict.clear()
...
def prepare_model_and_optimizer(args, device, stream, rng_state_tracker=None):
    global_step = 0
    args.resume_step = 0
    checkpoint = None
    ...
    config.eval_cuda_graph = args.eval_cuda_graph
    ...
    # FP8 recipe
    config.use_transformer_engine = args.use_transformer_engine
    config.use_transformer_engine2 = args.use_transformer_engine2
    if config.use_transformer_engine or config.use_transformer_engine2:
        config.te_rng_state_tracker = rng_state_tracker
    # Capture encoder graph for large batch
    if args.packed_samples and args.use_cuda_graph and args.train_batch_size>6:
        config.graph_capture_large_batch = True
    else:
        config.graph_capture_large_batch = False
    ...
    if args.init_checkpoint is not None or found_resume_checkpoint(args):
        ...
        if config.use_transformer_engine:
            checkpoint_remapped = remap_te_transformer_layer_parameters(checkpoint_remapped, config)
        elif config.use_transformer_engine2:
            checkpoint_remapped = remap_transformer_layer_parameters(checkpoint_remapped, config)
    ...
    if config.use_transformer_engine or config.use_transformer_engine2 or not (args.pad_fmha or args.unpad_fmha):
    ...
    mllogger.event(key=mllogger.constants.OPT_LAMB_EPSILON, value=optimizer.defaults['eps'], sync=False)
    mllogger.event(key="opt_epsilon", value=optimizer.defaults['eps'], sync=False)
    ...
    with torch.cuda.stream(stream):  
        ...
        if set_param_views_to_flat_buffer and (args.pad_fmha or args.unpad_fmha):
            ...
            for i, p in enumerate(optimizer._model_params):
                ...
                prev = p
            if not (config.use_transformer_engine or config.use_transformer_engine2):
                for i in range(24):
...
def run_graphed_eval_multiple(args, graph_eval, batch_placeholder, model, trainer, eval_dataloader, device, loss_eval, mlm_accuracy_eval, num_valid_eval, first_eval=False, use_cache=False):
    model.eval()
    ...
    with torch.no_grad():
        for batch in cached_batches if use_cache else eval_dataloader:
            ...
            if batch[0].size(0) == args.eval_batch_size:
                for t,t_gpu in zip(batch, batch_placeholder):
                    t_gpu.copy_(t, non_blocking=True)
                graph_eval.replay()
                total_eval_loss += loss_eval * num_valid_eval
                total_eval_mlm_acc += mlm_accuracy_eval * num_valid_eval
                total_masked += num_valid_eval
            else:
...
def main():
    ...
    if args.use_transformer_engine or args.use_transformer_engine2:
        _CUDA_RNG_STATE_TRACKER = TECudaRNGStatesTracker()
        _CUDA_RNG_STATE_TRACKER.add('model-parallel-rng', worker_seed)

    model, optimizer, lr_scheduler, checkpoint, global_step = prepare_model_and_optimizer(args, device, capture_stream, _CUDA_RNG_STATE_TRACKER)
    ...
    completed_ranks = torch.zeros((1,), dtype=torch.int, device=current_device) # number of Ranks that completed training
    ...
    if args.unpad:
        ...
        torch.cuda.synchronize()
    if args.train_batch_size>6 and not args.packed_samples:
        assert not args.use_cuda_graph, "training batch size larger than 6 is graph capturable only for packed data"
    ...
    if args.do_train:
        ...
        if found_resume_checkpoint(args):
            ...
            files = ['synth_data_0','synth_data_1']
            f_start_id = 0
        ...
        graph_capture_large_batch = args.train_batch_size>6 and use_cuda_graph
        use_DDP = args.use_ddp
        load_eval_synchronously = args.load_eval_synchronously

#        if utils.get_world_size() > 1:
#            assert use_DDP, "Running multi-GPU without --use_ddp"
        if utils.get_world_size() == 1:
            assert args.load_eval_synchronously, "Running on single GPU requires synchronous load of eval set" #TODO check why pool executor fails here
        ...
        if not args.packed_samples : # only a single graph/placeholder
            ...
            for seq_count in range(args.train_batch_size,args.train_batch_size*args.max_pack_factor+1):
                batch_gpu_placeholder = preprocess_batch(args,
                    ...
                    torch.ones(seq_count, dtype=torch.int64, device=device),
                    graph_capture_large_batch=(args.train_batch_size>6))
        ...
        if use_cuda_graph and args.cuda_graph_mode=='full_iteration':
            ...
            model = fwd_loss_bwd_trainer.capture_bert_model_segment_graph(model, use_cuda_graph, graph_capture_large_batch)
            current_device = torch.device('cuda', torch.cuda.current_device())
            # skipped_steps = torch.zeros((1,), dtype=torch.int, device=current_device)

        if use_cuda_graph and (args.cuda_graph_mode=='full_iteration' or args.eval_cuda_graph):
            # for fullcapture we want the eval to be graphed so each worker runs only one batch 
            # (that is copied once and cached in the buffer)
            if args.cuda_graph_mode=='full_iteration':
        ...
        if use_DDP and not args.distributed_lamb:
            ...
            if args.cuda_graph_mode=='segmented' or not use_cuda_graph:
                if use_cuda_graph and args.eval_cuda_graph:
                ...
                if use_cuda_graph:
                    if args.max_iterations_per_graph == 1: 
                        for seq_count in range(args.train_batch_size, args.train_batch_size*args.max_pack_factor+1):                   
                            graph = torch.cuda.CUDAGraph()
                    ...
                    # Need this if check as WAR to fix OOM for 64 node config
                    if args.max_iterations_per_graph > 1:
        ...
        if args.use_transformer_engine or args.use_transformer_engine2:
            ...
            reset_fp8_state(model.bert_model_segment.bert.encoder.layer)

        mllogger.log_init_stop_run_start()
        throughput_measure_time_start = time.time()

        # Start prefetching eval dataset
        if args.eval_dir:
            if load_eval_synchronously:
        ...
        now_step, now_skipped, skip_interval, now_epoch_num = 0, 0, 0, 0
        status = ''
        ...
        while global_step < args.max_steps and not end_training:
            ...
            if utils.is_main_process():
                ...
                now_time_log_freq = time.time()
                now_time_log_mlperf = now_time_log_freq
            ...
            if args.synthetic_input:
                data_file = None
            elif torch.distributed.is_initialized() and torch.distributed.get_world_size() > num_files:
            ...
            train_dataloader, data_file = create_pretraining_dataset(data_file, args.max_predictions_per_seq, shared_file_list, args, worker_init_fn=worker_init, synthetic_input=args.synthetic_input)
            ...
            for f_id in range(f_start_id + 1, len(files)):
                ...
                get_ntokens = args.packed_samples
                #torch.cuda.cudart().cudaProfilerStart()
                train_iterator = iter(train_dataloader)
                step = 0
                batch = next(train_iterator)
                batch = preprocess_batch(args, *batch, get_ntokens=get_ntokens, graph_capture_large_batch=graph_capture_large_batch)
                while True:
                    ...
                    if graph_capture_large_batch:
                        actual_batch = batch[-1].shape[0]
                    else:
                        actual_batch = batch[-2].shape[0]
                    #print ('actual_batch {}, ntokens {}'.format(actual_batch, ntokens))
                        ...
                        if forming_multi_iter_batch:
                            ...
                            if samples_in_multi_iter_batch<args.max_iterations_per_graph:                                
                                ...
                                if next_batch is not None:
                                    next_batch = preprocess_batch(args, *next_batch, get_ntokens=get_ntokens, graph_capture_large_batch=graph_capture_large_batch)
                                batch = next_batch
                                continue
                            else:
                                samples_in_multi_iter_batch=0
                    ...
                    if args.cuda_graph_mode=='segmented' or not use_cuda_graph:
                        ...
                        loss, mlm_acc, sbridge, next_batch = fwd_loss_bwd_trainer.step(step,
                                                                  batch,
                                                                  model,
                                                                  optimizer,
                                                                  sbridge,
                                                                  ntokens=ntokens,
                                                                  graph_capture_large_batch=graph_capture_large_batch,
                    ...
                    if update_step:
                        ...
                        if (args.eval_dir and args.eval_iter_samples > 0 and global_step == next_eval_step):
                            ...
                            if skip_interval > 0:
                                ...
                                if eval_count == 0 and not load_eval_synchronously:
                                ...
                                epoch_num=global_samples_trained.item()
                                if utils.is_main_process():
                                    time_interval = time.time() - now_time_log_mlperf
                                    now_time_log_mlperf = time.time()
                                    epoch_interval = epoch_num - now_epoch_num
                                    now_epoch_num = epoch_num
                                    training_perf = epoch_interval / time_interval
                                    mllogger.event(key='tracked_stats', 
                                                   value= {'throughput': training_perf},
                                                           metadata={mllogger.constants.EPOCH_NUM: epoch_num},
                                                           sync=False)

                                samples_trained_prev = samples_trained
                                mllogger.start(key=mllogger.constants.EVAL_START, value=epoch_num, sync=False, metadata={mllogger.constants.EPOCH_NUM: epoch_num})
                                if args.cuda_graph_mode=='segmented' or not use_cuda_graph:
                                    if use_cuda_graph and args.eval_cuda_graph:
                                        eval_avg_loss, eval_avg_mlm_accuracy = run_graphed_eval_multiple(args, graph_eval, batch_gpu_placeholder_eval, model, fwd_loss_bwd_trainer, eval_dataloader, device, loss_eval, mlm_accuracy_eval, num_valid_eval, first_eval=(eval_count == 0), use_cache=args.cache_eval_data)
                                    else:
                                ...
                                mllogger.end(key=mllogger.constants.EVAL_STOP, value=epoch_num, sync=False, metadata={mllogger.constants.EPOCH_NUM: epoch_num})


                                if utils.is_main_process():
                                    mllogger.event(key=mllogger.constants.EVAL_ACCURACY, value=eval_avg_mlm_accuracy, metadata={mllogger.constants.EPOCH_NUM: global_samples_trained.item()}, sync=False)
                                ...
                                if args.target_mlm_accuracy:
                                    if eval_avg_mlm_accuracy >= args.target_mlm_accuracy:
                                        if utils.is_main_process() and not converged:
                                            print("%f > %f, Target MLM Accuracy reached at %d"%(eval_avg_mlm_accuracy, args.target_mlm_accuracy, global_step))
                                        converged = True
                                        end_training = True if check_sustained_training_time(args.sustained_training_time, raw_train_start) else False

                                eval_count += 1
                                if not end_training and eval_count >= len(eval_steps):
                                    eval_count = 0 # reset eval_count to avoid IndexError
                                next_eval_step = eval_steps[eval_count]
                    ...
                    if args.log_freq > 0 and training_steps % (args.log_freq * args.gradient_accumulation_steps) == 0:
                        if utils.is_main_process():
                            time_interval = time.time() - now_time_log_freq
                            ...
                            # TODO: do we need to set skip_interval=0 here?
                            skip_interval = 0
                    ...
                    if not done_log_run_stop and (global_step >= args.max_steps or end_training or converged):
                    ...
                    if not done_log_run_stop and (converged or end_training or (samples_trained - samples_trained_prev >= args.num_samples_per_checkpoint and samples_trained >= args.min_samples_to_start_checkpoints)):
                        samples_trained_prev = samples_trained
                        ...
                        if samples_trained >= args.max_samples_termination or end_training or converged:
                            status = 'success' if converged else 'aborted'
                            end_training = True if check_sustained_training_time(args.sustained_training_time, raw_train_start) else False

                    if status != '':
                        if not done_log_run_stop and status != '':
                            ...
                            done_log_run_stop = True
                        end_training = True if check_sustained_training_time(args.sustained_training_time, raw_train_start) else False
                        if end_training:
                            completed_ranks.fill_(1)
                        torch.distributed.all_reduce(completed_ranks)
                        if end_training:
                            if completed_ranks.item() < torch.distributed.get_world_size():
                                end_training = False
                            else:
                                break
                ...
                if global_samples_trained >= args.max_samples_termination or end_training or converged:
                    status = 'success' if converged else 'aborted'
                    end_training = True if check_sustained_training_time(args.sustained_training_time, raw_train_start) else False
                    if end_training:
                        break
        ...
        # for DLFW CI/CD
        throughput_measure_time = time.time() - throughput_measure_time_start
        training_perf = global_samples_trained.item() / throughput_measure_time
        mllogger.event(key='tracked_stats', 
                                value= {'throughput': training_perf, 
                                        'epoch_num': global_samples_trained.item(), 
                                        },
