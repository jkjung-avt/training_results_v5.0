...
from mlperf_logging.mllog.constants import (RETINANET, STATUS, SUCCESS, ABORTED, INIT_START, INIT_STOP, RUN_START,
                                            RUN_STOP, SEED, GLOBAL_BATCH_SIZE, TRAIN_SAMPLES, EVAL_SAMPLES, EPOCH_COUNT,
...
try:
    ...
    print("Could not import NVIDIA DALI, it's fine if you do not use --dali")
...
def parse_args(add_help=True):
    ...
    parser.add_argument('--dali-pinned-memory-size', default=256*1024*1024, type=int,
                        help='preallocated pinned memory size in bytes')
    ...
    parser.add_argument('--dali-resize-first', action="store_true",
                        help='resize input images first')
    ...
    parser.add_argument('--syn-dataset', dest='syn_dataset', type=int, default=0,
                        help='use synthetic dataset (0: use actual dataset, 1: reuse batch created from actual dataset, 2: reuse fully synthetic batch)')
    ...
    parser.add_argument("--not-power-benchmark", dest='power_benchmark', action="store_false",
                        help="If this is a power benchmark run, training epochs will increase by "
                             "10x and won't stop when the target mAP is achieved.")
    parser.set_defaults(power_benchmark=(os.getenv('MLPERF_POWER_TRAIN_AFTER_RUN_STOP', '0') == '1'))
...
def main(args):
    ...
    assert((args.dali and args.syn_dataset == 0) or not args.dali)
    # Do not do eval when using fully synthetic data
    assert((args.skip_eval and args.syn_dataset == 2) or args.syn_dataset != 2)
    ...
    train_group, eval_group, train_group_gloo, device = utils.init_distributed_mode(args)

    # Start MLPerf benchmark
    mllogger.mlperf_submission_log(benchmark=RETINANET)
    ...
    model = retinanet_from_backbone(backbone=args.backbone,
                                    ...
                                    pretrained_backbone=True)  # TODO: set to =args.syn_dataset < 2, but first fix FrozenBN and maybe other layers which set new fields when load_state_dict is called
    model.to(device)
    ...
    # Preallocate host pinned memory for DALI
    nvidia.dali.backend.PreallocatePinnedMemory(args.batch_size * args.dali_pinned_memory_size)

    # The dali based data_loader doesn't touch data at init time (lazy_init=True). So we place before RUN_START
    if args.dali and (args.rank in args.train_ranks) and (not args.test_only):
        ...
        data_loader = DaliDataIterator(data_path=args.train_data_path,
                                       ...
                                       sync=args.dali_sync,
                                       resize_first=args.dali_resize_first,
                                       seed=args.seed)
    ...
    if not args.dali and (args.rank in args.train_ranks) and (not args.test_only):
        ...
        if args.syn_dataset == 2:
            print("Using fully synthetic dataset")
            data_loader = get_cached_dataset(model_without_ddp, device, args, dataset=None)
        else:
            dataset = dataset_fn(dataset_path=args.train_data_path,
                                 ...
                                 training=True)
            if args.syn_dataset == 1:
                print("Using synthetic dataset created with actual dataset")
                data_loader = get_cached_dataset(model_without_ddp, device, args, dataset=dataset)
            else:
    ...
    if not args.dali and (args.rank in args.eval_ranks) and not args.skip_eval:
    ...
    if args.cuda_graphs_eval and not args.cuda_graphs_syn and not args.skip_eval:
        graphed_model_eval, static_input_eval, static_model_output_eval = \
    ...
    def stop_training(args, status, start_time, check_power_sustain_time):
        """
    ...
    if args.test_only and (args.rank in args.eval_ranks):
        ...
        for epoch in range(args.start_epoch, args.epochs):
            ...
            if args.rank in args.train_ranks:
                if args.distributed and not args.dali and args.syn_dataset == 0:
                ...
                metric_logger, status = train_one_epoch(model=model,
                                                        ...
                                                        train_group_gloo=train_group_gloo,
                                                        args=args,
                ...
                if stop_training(args, status, start_time, False):
                    break
            ############################################################################################################
            ...
            if args.rank in args.eval_ranks and not args.skip_eval:
                ...
                # check if we should stop training
                if stop_training(args, status, start_time, True):
                    break
            ############################################################################################################
    ...
    if args.async_coco:
        while status != SUCCESS and len(async_executor.tags()):
            ...
            if args.distributed:
                accuracy = utils.broadcast(accuracy, 0, group=train_group_gloo)
...
if __name__ == "__main__":
    ...
    # workaround for the problem of rank 0 hanging very long (likely due to async_executor)
    import os, time
    time.sleep(60)
    os._exit(0)
